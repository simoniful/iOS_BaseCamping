...
class DaejeonViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["대전광역시"]![0]))&lon=\(String(describing: WeatherStation.locationDic["대전광역시"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension DaejeonViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension DaejeonViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

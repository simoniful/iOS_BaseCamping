...
class GangwonViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["강원도"]![0]))&lon=\(String(describing: WeatherStation.locationDic["강원도"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension GangwonViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension GangwonViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

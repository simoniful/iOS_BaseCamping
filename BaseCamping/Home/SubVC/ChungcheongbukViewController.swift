//
//  ChungcheongbukViewController.swift
//  BaseCamping
//
...
class ChungcheongbukViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["충청북도"]![0]))&lon=\(String(describing: WeatherStation.locationDic["충청북도"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension ChungcheongbukViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension ChungcheongbukViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

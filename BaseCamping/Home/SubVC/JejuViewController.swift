...
class JejuViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["제주특별자치도"]![0]))&lon=\(String(describing: WeatherStation.locationDic["제주특별자치도"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension JejuViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension JejuViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

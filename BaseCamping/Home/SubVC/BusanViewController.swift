...
class BusanViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["부산광역시"]![0]))&lon=\(String(describing: WeatherStation.locationDic["부산광역시"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension BusanViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension BusanViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

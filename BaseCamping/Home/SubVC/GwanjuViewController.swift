...
class GwanjuViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["광주광역시"]![0]))&lon=\(String(describing: WeatherStation.locationDic["광주광역시"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension GwanjuViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension GwanjuViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

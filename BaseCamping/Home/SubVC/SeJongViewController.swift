...
class SeJongViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["세종특별자치시"]![0]))&lon=\(String(describing: WeatherStation.locationDic["세종특별자치시"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension SeJongViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension SeJongViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

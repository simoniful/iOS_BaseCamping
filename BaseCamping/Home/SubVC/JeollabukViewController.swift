...
class JeollabukViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["전라북도"]![0]))&lon=\(String(describing: WeatherStation.locationDic["전라북도"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension JeollabukViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension JeollabukViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

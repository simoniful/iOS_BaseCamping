...
class DaeguViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["대구광역시"]![0]))&lon=\(String(describing: WeatherStation.locationDic["대구광역시"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension DaeguViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension DaeguViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

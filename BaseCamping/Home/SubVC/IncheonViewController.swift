...
class IncheonViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["인천광역시"]![0]))&lon=\(String(describing: WeatherStation.locationDic["인천광역시"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension IncheonViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension IncheonViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

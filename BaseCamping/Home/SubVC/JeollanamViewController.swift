...
class JeollanamViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["전라남도"]![0]))&lon=\(String(describing: WeatherStation.locationDic["전라남도"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension JeollanamViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension JeollanamViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {

//
//  GyeongsangnamViewController.swift
//  BaseCamping
//
...
class GyeongsangnamViewController: UIViewController {
    ...
    @objc func fetchWeatherInfo() {
        let url = "\(Endpoint.weatherURL)?lat=\(String(describing: WeatherStation.locationDic["경상남도"]![0]))&lon=\(String(describing: WeatherStation.locationDic["경상남도"]![1]))&exclude=current,minutely,hourly,alerts&appid=\(APIKey.weather)&units=metric&lang=kr"
...
extension GyeongsangnamViewController: FSPagerViewDelegate,FSPagerViewDataSource {
...
extension GyeongsangnamViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
